...
for(i in names(markers_hom)){
    colnames(markers_hom[[i]]) <-  c("Mouse_ensembl","Human_Ensembl",
...
save(markers_hom,
     file = here("MAGMA",
                 "mouse_analysis",
                 "mouse_markers_human_homologs_list.rda"))
...
#Column 2 needs to be human entrez_geneid. 
for(i in names(markers_hom)){
    markers_hom[[i]]$Set <- i
