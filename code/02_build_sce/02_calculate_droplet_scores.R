...
for(i in unique(sce$Sample)){
    ...
    save(e.out,file = here("processed-data","02_build_sce","droplet_scores",paste0(i,"_droplet_scores.Rdata")))
    
    #Generate plots to check the cutoff value. 
    FDR_cutoff <- 0.001
    print(addmargins(table(Signif = e.out$FDR <= FDR_cutoff, Limited = e.out$Limited,useNA = "ifany")))
   ...
   ggsave(droplet_elbow_plot,
       filename = here("plots","02_build_sce","droplet_score",paste0(i,"_droplet_qc.png")))
